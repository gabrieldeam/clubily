...
export default function MapsScreen() {
  ...
  // Obtém bounding box da cidade atual para o aviso de "fora da área"
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (data[0]?.boundingbox) {
          const [south, north, west, east] = data[0].boundingbox.map((v: string) =>
            parseFloat(v)
          );
  ...
  // Busca categorias + empresas + geocodifica cada empresa para colocar no mapa
  useEffect(() => {
    if (!selectedAddress) return;
    ...
    (async () => {
      ...
      try {
        ...
        const [catsRes, compsRes] = await Promise.all([catsPromise, compsPromise]);
        setCategories(catsRes.data);

        // Geocodifica cada empresa (temporário, só pra posicionar no mapa)
        const withCoords: CompanyWithCoords[] = await Promise.all(
        ...
        // Se houver pelo menos uma empresa com coords, centraliza na primeira
  ...
  // Se o usuário só mover o mapa (zoom/pan), verifica se ainda está dentro da cidade
  useEffect(() => {
    ...
    <View style={styles.container}>
      ...
      <UniversalMap
        ...
        companies={companies.map((c) => ({
          id: c.id,
          coords: c.coords,
          logo: `${imagePublicBaseUrl}${c.logo_url}`,
          name: c.name,
          street: c.street,
          city: c.city,
          state: c.state,
