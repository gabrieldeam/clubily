// src/app/programs/cashback/[program_id]/page.tsx
'use client';
...
export default function ProgramDetailPage() {
  // Next.js useParams returns Record<string, string | string[] | undefined>
  const rawParams = useParams() as Record<
  ...
  const programIdRaw =
    (Array.isArray(rawParams.programId)
      ? rawParams.programId[0]
      : rawParams.programId) ??
    (Array.isArray(rawParams.program_id)
      ? rawParams.program_id[0]
      : rawParams.program_id);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const progResp = await getCashbackProgram(programId);
        const progData =
          'data' in progResp ? progResp.data : progResp;
        setProgram(progData);

        // busca uso com paginação
        const usageResp = await getProgramUsage(
          programId,
          page * limit,
          limit
        );
        const usageData =
          'data' in usageResp ? usageResp.data : usageResp;
        setUsage(usageData);
      } catch (error) {
        console.error(error);
          ...
          <section className={styles.header}>
            ...
            <div className={styles.metrics}>
              ...
              <div>
                ...
                <strong>{usage.unique_user_count.toFixed(0)}</strong>
              </div>
              <div>
              ...
              <div>
                ...
                <strong>
                  {usage.average_interval_days?.toFixed(2) ?? '0.00'}
                </strong>
              </div>
            </div>
            <div className={styles.props}>
              <div>
                <strong>Percentual</strong> {program.percent}%
              </div>
              <div>
                <strong>Validade</strong> {program.validity_days}{' '}
                dia{program.validity_days > 1 ? 's' : ''}
              </div>
              <div>
                <strong>Status</strong> {program.is_active ? 'Ativo' : 'Inativo'}
              </div>
              <div>
                <strong>Visível</strong> {program.is_visible ? 'Sim' : 'Não'}
              </div>
            </div>
          </section>
                ...
                <div className={styles.tableBody}>
                  {usage.associations.map(a => (
                    <div key={a.id} className={styles.tableRow}>
                      ...
                      <div
                        className={styles.colGasto}
                        data-label="Gasto (R$):"
                      >
                        {a.amount_spent.toFixed(2)}
                      </div>
                      <div
                        className={styles.colCashback}
                        data-label="Cashback (R$):"
                      >
                        {a.cashback_value.toFixed(2)}
                      </div>
                      <div
                ...
                {usage.associations.map(a => (
                  <div key={a.id} className={styles.card}>
                    ...
                    <div className={styles.cardBody}>
                      <p>
                        <span>Gasto:</span> R$ {a.amount_spent.toFixed(2)}
                      </p>
                      <p>
                        <span>Data:</span>{' '}
                        {new Date(a.assigned_at).toLocaleDateString()}
