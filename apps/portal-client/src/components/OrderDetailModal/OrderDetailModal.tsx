...
export default function OrderDetailModal({ order, onClose }: Props) {
  ...
  const grouped = order.items.reduce<
    Record<string, { product: RewardOrderRead['items'][0]['product']; quantity: number }>
  >((acc, cur) => {
  ...
  const totalPoints = items.reduce(
    (sum, i) => sum + i.product.points_cost * i.quantity,
    0
  );
  ...
  return (
    <Modal open onClose={onClose} width={700}>
      <div className={styles.container}>
        ...
        {order.refusal_msg && (
          <p className={styles.refusalMsg}>
            Motivo da recusa: {order.refusal_msg}
          </p>
        )}
              ...
              <li key={i.product.id} className={styles.item}>
                <div className={styles.thumbnailWrapper}>
                  <Image
                    ...
                    unoptimized={!i.product.image_url} // placeholder sem otimização
                  />
                </div>
